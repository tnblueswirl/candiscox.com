...
function thegem_scripts() {
	...
	if(thegem_get_option('custom_js')) {
		wp_add_inline_script('thegem-scripts', stripslashes(thegem_get_option('custom_js')));
	}
	...
	if(is_404() && get_post(thegem_get_option('404_page')) && $page_404_inline_style = get_post_meta(thegem_get_option('404_page'), '_wpb_shortcodes_custom_css', true)) {
...
function thegem_open_graph() {
	...
	if (is_singular(array('post', 'thegem_pf_item'))) {
		// title
...
function thegem_google_fonts_load_file() {
	...
	$font_file = thegem_get_option('google_fonts_file');
	if(!file_exists($font_file)) {
		$font_file = get_template_directory_uri() . '/fonts/webfonts.json';
	}
	$fontsList = false;
	$font_json_file = wp_remote_get(get_template_directory_uri() . '/fonts/webfonts.json', array('timeout' => 30));
	if(!is_wp_error($font_json_file)) {
		$fontsList = json_decode($font_json_file['body']);
	}
...
function thegem_post_thumbnail($size = 'thegem-post-thumb', $dummy = true, $class='img-responsive img-circle') {
...
function thegem_image_sizes() {
	return apply_filters('thegem_image_sizes', array(
		...
		'thegem-portfolio-metro' => array(0, 800, false),
		...
		'thegem-gallery-justified-double-4x' => array(880, 800, true),
		'thegem-gallery-justified-double-4x-horizontal' => array(880, 400, true),
		'thegem-gallery-justified-double-4x-vertical' => array(440, 870, true),
		...
		'thegem-gallery-metro' => array(0, 800, false),
...
$text['404'] = esc_html__('Posts by', 'thegem');
$text['page'] = '%s';
	...
	$cat = get_category(get_query_var('cat'), false);
		...
		elseif ( is_single() && !is_attachment() ) {
			...
			if ( get_post_type() != 'post' ) {
				...
				{
					echo  sprintf($link, get_permalink(get_option ('woocommerce_shop_page_id' , 0 )), 'Shop');
				}
				else {
					$slug = $post_type->rewrite;
...
function thegem_vc_btn($a, $b){
	return thegem_button(array('extra_class' => 'vc_btn3', 'text' => __('Load more...', 'thegem'), 'background_color' => '#00bcd4', 'separator' => 'soft-double', 'size' => 'medium'));
}
add_filter('vc_gitem_template_attribute_vc_btn', 'thegem_vc_btn', 11, 2);
...
function thegem_page_scroller_menu($value) {
	if(is_singular()) {
		$page_effects = thegem_get_sanitize_page_effects_data(get_the_ID());
		if($page_effects['effects_page_scroller']) {
			return 'fullwidth_hamburger';
		}
	}
	return $value;
}
add_filter('thegem_option_header_layout', 'thegem_page_scroller_menu');
...
function thegem_vertical_fix_logo_position($value) {
	if(($value == 'menu_center' || $value == 'center') && (thegem_get_option('header_layout') == 'fullwidth_hamburger' || thegem_get_option('header_layout') == 'vertical')) {
		return 'left';
	}
	return $value;
}
