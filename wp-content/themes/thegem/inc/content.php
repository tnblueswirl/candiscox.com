...
function thegem_contacts() {
	...
	if(thegem_get_option('contacts_address')) {
		$output .= '<div class="gem-contacts-item gem-contacts-address"> Address: </br> '.esc_html(stripslashes(thegem_get_option('contacts_address'))).'</div>';
	}
			...
			<div class="related-posts-carousel">
				<?php while ($related_query->have_posts()) : $related_query->the_post(); ?>
					<div class="related-element">
						<a href="<?php echo esc_url(get_permalink()); ?>"><?php thegem_post_thumbnail('thegem-post-thumb', true, ''); ?></a>
...
function thegem_blog($params = array()) {
	...
	if($posts->have_posts()) {
		if ($params['blog_style'] == 'grid_carousel') {
			...
			wp_enqueue_script('thegem-news-carousel');
			echo '<div class="preloader"><div class="preloader-spin"></div></div>';
			echo '<div class="gem-blog-slider gem-blog-slider-style-'.$slider_style.' clearfix"'.(intval($params['slider_autoscroll']) ? ' data-autoscroll="'.intval($params['slider_autoscroll']).'"' : '').'>';
			while($posts->have_posts()) {
				$posts->the_post();
...
function thegem_get_post_featured_content($post_id, $thumb_size = 'thegem-blog-default', $single = false) {
	...
	if($post_item_data['show_featured_content'] || !$single) {
		if($format == 'video' && $post_item_data['video']) {
			...
			if($post_item_data['video_type'] == 'youtube') {
				$video_block = '<iframe frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="'.esc_url('//www.youtube.com/embed/'.$post_item_data['video'].'?rel=0&amp;wmode=opaque').'"></iframe>';
			...
			$img = thegem_generate_thumbnail_src(get_post_thumbnail_id($post_id), $thumb_size);
			$image = '<img src="'.esc_url($img[0]).'" alt="#" class="img-responsive" />';
			if($single) {
				$output = $image;
			} else {
