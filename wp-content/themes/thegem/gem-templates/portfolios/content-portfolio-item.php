...
if (isset($thegem_portfolio_item_data['highlight']) && $thegem_portfolio_item_data['highlight'] && empty($params['is_slider']) && $thegem_highlight_type != 'squared') {
...
$thegem_small_image_url = apply_filters('portfolio_small_image_filter', thegem_generate_thumbnail_src(get_post_thumbnail_id(), $thegem_size));
	...
	<div class="wrap clearfix">
		<div <?php post_class($thegem_image_classes); ?>>
			<div class="image-inner">
				<img src="<?php echo esc_url($thegem_small_image_url[0]); ?>" width="<?php echo esc_attr($thegem_small_image_url[1]); ?>" <?php if($thegem_small_image_url[2]): ?>height="<?php echo esc_attr($thegem_small_image_url[2]); ?>"<?php endif; ?> <?php if(!empty($thegem_small_image_url['srcset'])): ?>srcset="<?php $thegem_srcset_index = 0; foreach($thegem_small_image_url['srcset'] as $thegem_srcset_key=>$thegem_srcset_url): $thegem_srcset_index++; ?><?php echo ($thegem_srcset_index > 1 ? ', ' : '') . esc_attr($thegem_srcset_url) . ' ' . $thegem_srcset_key; ?><?php endforeach; ?>"<?php endif; ?> alt="<?php the_title(); ?>" />
			</div>
			<div class="overlay">
				...
				<?php if (count($thegem_portfolio_item_data['types']) == 1 && $params['disable_socials']): ?>
					<?php
						...
						if($thegem_ptype['type'] == 'full-image') {
							...
							$thegem_link = 'http://www.youtube.com/embed/'.$thegem_ptype['link'].'?autoplay=1';
						} elseif($thegem_ptype['type'] == 'vimeo') {
							$thegem_link = 'http://player.vimeo.com/video/'.$thegem_ptype['link'].'?autoplay=1';
						} else {
