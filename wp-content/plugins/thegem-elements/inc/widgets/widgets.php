...
add_action('widgets_init', 'thegem_widgets_init');
	...
	class The_Gem_Project_Slider extends WP_Widget
	{
		...
		{
			$widget_ops = array('Project_Slider' => 'widget_project_slider', 'description' => __('Project Slider', 'thegem'));
			parent::__construct('project_slider', __('Project Slider', 'thegem'), $widget_ops);
		}
		...
		function form($instance) {
			...
			<br/>
			<p><label for="<?php echo $this->get_field_id('rows'); ?>"><?php _e('Rows', 'thegem'); ?>: <input
...
function pp_posts($sort = 'recent', $items = 3, $echo = TRUE, $categories = array()) {
	...
	if (!empty($posts)) {
		...
		foreach ($posts as $post) {
			$image_thumb = '';
			if (has_post_thumbnail($post->ID, 'thegem-post-thumb')) {
				$image_id = get_post_thumbnail_id($post->ID);
				$image_thumb = wp_get_attachment_image_src($image_id, 'thegem-post-thumb', true);
			}
			$return_html .= '<li class="clearfix gem-pp-posts">';
			if (!empty($image_thumb)) {
				$return_html .= '<div class="gem-pp-posts-image"><a href="' . get_permalink($post->ID) . '"><img src="' . $image_thumb[0] . '" alt=""/></a></div>';
			} else {
...
class The_Gem_Widget_Recent_Posts extends WP_Widget {
	function The_Gem_Widget_Recent_Posts() {
		...
		if ($params['style'] == 0) {
			...
			while ($testimonials->have_posts()) {
				...
				<div class="testimonials-style-1-item ">
					<div class="testimonials-style-1-image"> <?php echo  $link_start; ?> <?php if($params['effects_enabled']): ?><span class="lazy-loading-item" style="display: block;" data-ll-item-delay="0" data-ll-effect="clip"><?php endif; ?> <?php  thegem_post_thumbnail('thegem-post-thumb'); ?> <?php if($params['effects_enabled']): ?></span><?php endif; ?> <?php echo $link_end; ?> </div>
	...
	while ($loop->have_posts()) : $loop->the_post();
		...
		<li class="gem-latest-news">
			<div class="gem-latest-news-image">
				<a href='<?php the_permalink() ?>'> <?php thegem_post_thumbnail('thegem-post-thumb', true, '') ?> </a>
			</div>
...
class The_Gem_Widget_Flickr extends WP_Widget {
	function The_Gem_Widget_Flickr() {
...
function thegem_teams($params) {
	...
	while ($loop->have_posts()) : $loop->the_post();
		...
		<div class="gem-teams-item rounded-corners">
			<span class="gem-teams-image"><?php echo $link_start ?> <?php if($params['effects_enabled']): ?><span class="lazy-loading-item" style="display: block;" data-ll-item-delay="0" data-ll-effect="clip"><?php endif; ?> <?php thegem_post_thumbnail('thegem-post-thumb'); ?> <?php if($params['effects_enabled']): ?></span><?php endif; ?> <?php echo $link_end; ?> </span>
...
?>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'rows' ) ); ?>"><?php _e( 'Rows count:', 'thegem' ); ?></label>
