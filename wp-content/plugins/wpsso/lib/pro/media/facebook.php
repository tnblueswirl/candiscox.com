	...
	class WpssoProMediaFacebook {
		...
		public function filter_video_info( $og_video, $embed_url, $embed_width = 0, $embed_height = 0 ) {
			...
			if ( ! empty( $og_video['og:video:secure_url'] ) || ! empty( $og_video['og:video:url'] ) ) {
				...
				if ( $this->p->debug->enabled )
					$this->p->debug->log( 'exiting early: not a facebook embed url' );
				return $og_video;
			}
			...
			if ( preg_match( '/^.*(facebook\.com)\/plugins\/video.php\?href=([^\/\?\&\#<>]+).*$/', $embed_url, $match ) ) {
				...
				if ( preg_match( '/"(hd|sd)_src_no_ratelimit":"([^"]+)"/', $video_html, $match ) ) {
					...
					if ( ! empty( $this->p->options['add_meta_property_og:video:secure_url'] ) )
						$og_video['og:video:secure_url'] = strpos( $mp4_url, 'https:' ) === 0 ? $mp4_url : '';

					$og_video['og:video:url'] = $mp4_url;
				...
				if ( preg_match( '/<img .* src="([^"]+\.jpg[^"]*)"/', $video_html, $match ) ) {
					...
					if ( ! empty( $this->p->options['add_meta_property_og:image:secure_url'] ) )
						$og_video['og:image:secure_url'] = strpos( $mp4_url, 'https:' ) === 0 ? $img_url : '';
