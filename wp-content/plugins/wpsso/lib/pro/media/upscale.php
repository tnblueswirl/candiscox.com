			...
			if ( ! empty( $img_info['size_name'] ) ) {
				if ( $this->p->notice->is_admin_pre_notices() ) {	// skip if notices already shown
					...
					if ( $size_diff > $max_diff ) {
						...
						$this->p->notice->warn( sprintf( __( 'Image ID %1$s of %2$s cannot be upscaled by %3$s from %4$s to %5$s for the %6$s image size (exceeds %7$s maximum).', 'wpsso' ), $img_info['pid'], $orig_w.'x'.$orig_h, $size_diff.'%', $src_w.'x'.$src_h, $dst_w.'x'.$dst_h, '<b>'.$size_label.'</b>', $max_diff.'%' ), true, $msg_id, true );
