			...
			if ( preg_match( '/^.*(wistia\.net|wistia\.com|wi\.st)\/([^\?\&\#<>]+).*$/i', $embed_url, $match ) ) {
				...
				if ( function_exists( 'simplexml_load_string' ) ) {
									...
									switch ( $meta_type.'-'.$meta_name ) {
										...
										case 'itemprop-embedURL':
											if ( ! empty( $a['content'] ) ) {
												...
												} else $og_video['og:video:url'] = preg_replace( '/^http:\/\/embed\./',
													'https://embed-ssl.', $a['content'] );
											}
											break;
					...
					if ( ! empty( $xml->thumbnail_url ) ) {
						...
						// use the https url by default
						} else $og_video['og:image'] = preg_replace( '/^http:\/\/embed\./', 'https://embed-ssl.', $img_url );
				...
				$this->p->debug->log( 'embed url does not match a known Wistia video URL' );
