...
if ( ! class_exists( 'WpssoProEcomYotpoWc' ) ) {
	...
	class WpssoProEcomYotpoWc {
		...
		public function filter_og_woocommerce_product_page( $og_ecom, $mod ) {
			...
			if ( ! apply_filters( $this->p->cf['lca'].'_og_add_product_mt_rating', true, $mod ) ) {
				if ( $this->p->debug->enabled )
			...
			if ( $plugin_dir !== false ) {
				$api_lib = realpath( $plugin_dir.'/'.$slug.'/lib/yotpo-api/Yotpo.php' );
			...
			if ( $have_error ) {
				if ( $this->p->debug->enabled )
					$this->p->debug->log( 'exiting early for error' );
				return $og_ecom;
			}
			...
			if ( class_exists( 'Yotpo' ) ) {
				...
				if ( $this->p->debug->enabled )
					$this->p->debug->log( 'calling Yotpo API get_product_bottom_line() method' );
				...
				$this->p->debug->log( 'error: Yotpo API class does not exist' );
