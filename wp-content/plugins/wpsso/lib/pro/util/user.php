		...
		public function get_options( $user_id, $idx = false, $filter_options = true ) {
			...
			if ( empty( $this->opts[$user_id]['options_filtered'] ) ) {
				...
				if ( ! empty( $this->opts[$user_id] ) && 
					...
					$this->p->util->rename_keys_by_ext( $this->opts[$user_id],
				...
				if ( $filter_options ) {
					...
					$this->opts[$user_id] = apply_filters( $lca.'_get_user_options',
				...
				$this->p->debug->log( 'using options from property cache for user_id '.$user_id );
