		...
		public function __construct( $options ) {
			try {
				if ( ! array_key_exists( 'key', $options ) || empty( $options['key'] ) ) {
					throw new SucomException( 'Ow.ly Error: API key missing.' );
				} else {
		...
		private function send( $apiUrl, $args = array(), $method = 'GET' ) {
			try {
				...
				if ( $method === self::HTTP_METHOD_GET ) {
					...
					throw new SucomException( 'Ow.ly Error: Invalid request method.' );
				}
				...
				if ( $response === false ) {
					throw new SucomException( 'Ow.ly Error: Invalid response received.' );
					...
					$errorMsg = 'Ow.ly Error: '.$apiUrl;
					if ( ! empty( $response ) ) {
						$data = json_decode( $response );
						$errorMsg .= ' '.$data->error;
					}
					throw new SucomException( $errorMsg, $metadata['http_code'] );
				}
