...
if ( ! class_exists( 'SuextBitly' ) ) {
	...
	class SuextBitly {
		...
		private function doCall( $url, $aParameters = array() ) {
			...
			try {
				if ( ! in_array( $headers['http_code'], array( 0, 200 ) ) ) {					// invalid headers
					throw new SucomException( 'Bitly Error: ', $headers['http_code'] );
				} elseif ( $errorNumber != '' ) {								// error
					throw new SucomException( 'Bitly Error: '.$errorMessage );
				} elseif ( ( $json = @json_decode( $response, true ) ) === false ) {				// validate json
					throw new SucomException( 'Bitly Error: Invalid JSON Response.' );
					...
					if( isset( $json['status_code'] ) && isset( $json['status_txt'] ) )			// bitly-error
						throw new SucomException( 'Bitly Error: '.(string) $json['status_txt'] );
					else throw new SucomException( 'Bitly Error: Invalid JSON Response.' );			// invalid json
				}
		...
		public function expand( $shortURL = null, $hash = null ) {
			...
			try {
				if( $shortURL == '' && $hash == '' ) {	// validate
					throw new SucomException( 'Bitly Error: shortURL or hash must have a value.' );
				}
			} catch ( SucomException $e ) {
				return $e->errorMessage();
			}
		...
		public function shorten( $url, $domain = null, $endUserLogin = null, $endUserApiKey = null ) {
			...
			try {
				if ( $domain !== null && ! in_array( (string) $domain, array( 'bit.ly', 'j.mp' ) ) ) {	// domain specified
					throw new SucomException( 'Bitly Error: Custom domains handled by using correct login.' );
				}
			} catch ( SucomException $e ) {
				return $e->errorMessage();
			}
